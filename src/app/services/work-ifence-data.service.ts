...
import { DealDataItem, EmailSubscription, GetDealsRequestParam, Merchant } from './deals.model';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { isPlatformBrowser, isPlatformServer } from '@angular/common';
import { HttpWrapperService } from './http-wrapper.service';
import { AppConstantsService } from './app-constants.service';
import { JobFeedItem } from './ifence.model';
import { ResumeListDataItem } from './work-ifence-data.model';
...
export class WorkIfenceDataService {
  ...
  getResumesByUserName() {
    return this.http.get<any>('assets/demo/data/resumes.json')
        .toPromise()
        .then(res => res.data as ResumeListDataItem[])
        .then(data => data);
  }
  ...
  getDealsByCountry(country: string, envi: object) {
    // let baseUrl = envi.toString() === 'server'?this.appConstants.BASE_SSR_API_URL:this.appConstants.BASE_API_URL;
    let baseUrl = this.appConstants.BASE_API_URL;
    if(isPlatformBrowser(this.platformId)){
      baseUrl = '';
